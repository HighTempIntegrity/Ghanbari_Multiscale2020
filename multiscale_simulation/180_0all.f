     ...
     1      COORDS(1) .GT. GLOBAL_LENGTH + SMOLL) THEN
      ...
      IF (STEP.EQ.ZERO) THEN
        IF (y_cur.LT.Y0+SMOLL .AND. y_cur.GT.Y0-THICKNESS+SMOLL) THEN
            IF (x_cur.LT.X0+TWO*RADIUS-SMOLL .AND. x_cur.GT.X0-TWO*RADIUS+SMOLL) THEN
                Q0 = 19.D0*ABSORB*POWER/(40.D0*RADIUS*WIDTH*DELTA)
                FLUX(1) = Q0*(ONE-(ABS(x_cur-X0)/(TWO*RADIUS)))
            END IF 
        END IF 
      ! Local
      ELSE
...
!====== Parameter Initialization ======!
      ...
      !=== Local Model | Step 2 ===!
      IF (k_step.EQ.TWO .AND. STEP.NE.ZERO) THEN
        FIELD(1,3) = ONE ! Changing FV2 for FV1 preparation
        RETURN
      END IF
...
!====== FVD Initialization ======!
      ...
      ! Local
      ELSE
        IF (KINC.EQ.ONE .AND. k_step.EQ.ONE .AND.                                 ! Beginning of 1st local step
     1     (STEP.EQ.ONE .OR.  FIELD(1,1).GT.5.D0 .OR. FIELD(1,1).LT.5.D-1)) THEN  ! Either first model, non-converted high FV
            init_fld = FIELD(1,1)
            ! Below the first layer
            IF      (y_cur .LT. ZERO+SMOLL) THEN                            ! Below print layers
                init_fld = ONE
            ELSE IF (y_cur .LT. THICKNESS*(layer_number-ONE)+SMOLL) THEN    ! Previous layers
                IF  (x_cur .LT. GLOBAL_LENGTH+SMOLL 
     1         .AND. x_cur .GT. ZERO-SMOLL) THEN    ! Heated region
                    CALL IS_HEATED(x_cur,y_cur,heated_state)
                    IF (heated_state) THEN
                        init_fld = ONE	! Solidified previous layers
                    END IF
                END IF
            END IF
            ...
            ELSE IF (FIELD(1,1).GT.PHASE_T_SLD) THEN
                init_fld = ONE	! Solid
            ELSE IF (FIELD(1,1).GT.PHASE_T_PWD) THEN
...
!====== Melting ======!
     ...
     1 T(3,3),TIME(2)
      ...
      REAL*8 x_cur,y_cur,k_step,layer_number,total_time
      
!====== Parameter Initialization ======!
      x_cur = COORD(1)      ! Current X coordinate
